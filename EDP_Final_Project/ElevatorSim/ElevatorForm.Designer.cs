...
namespace ElevatorSim
{
    partial class ElevatorForm
    {
        ...
        private void InitializeComponent()
        {
            ...
            {
                BackColor = Color.FromArgb(232, 228, 218),
            ...
            rightOuter.RowStyles.Add(new RowStyle(SizeType.Percent, 57f));   // status + floor sel
            rightOuter.RowStyles.Add(new RowStyle(SizeType.Percent, 25f));   // elevator controls
            rightOuter.RowStyles.Add(new RowStyle(SizeType.Percent, 18f));   // system controls
            ...
            var statusHeader = new Label
            {
                ...
                BackColor = Color.FromArgb(30, 44, 80),
                Dock = DockStyle.Top,
                Height = 32,
            ...
            // Inner content panel — keeps everything inside the LCD box
            ...
            var cfCaption = new Label
            {
                ...
                ForeColor = Color.FromArgb(0, 220, 70),
                BackColor = Color.Transparent,
                TextAlign = ContentAlignment.MiddleCenter,
                Dock = DockStyle.None
            };
            ...
            // Layout labels inside the LCD content panel
            void LayoutLcd()
            {
            ...
            // Also run immediately so labels are sized before first paint
            statusPanel.VisibleChanged += (_, _) => { if (statusPanel.Visible) LayoutLcd(); };

            // ── Floor selection ───────────────────────────────────────────
            var floorSelPanel = new Panel
            {
                BackColor = Color.FromArgb(20, 28, 58),
                Dock = DockStyle.Fill,
            ...
            var floorHeaderTop = new Label
            {
                ...
                ForeColor = Color.FromArgb(160, 180, 225),
                BackColor = Color.FromArgb(14, 20, 42),
                Dock = DockStyle.Top,
                Height = 14,
                TextAlign = ContentAlignment.MiddleCenter
            };
            ...
            for (int floor = TotalFloors; floor >= 1; floor--)
            {
                var btn = new Button
                {
                    ...
                    BackColor = Color.FromArgb(48, 68, 118),
                    ForeColor = Color.White,
                    FlatStyle = FlatStyle.Flat,
                    Tag = floor,
                    Cursor = Cursors.Hand
                };
                btn.FlatAppearance.BorderColor = Color.FromArgb(90, 120, 195);
            ...
            var ctrlOuter = new TableLayoutPanel
            {
                ...
                ColumnCount = 1,
                RowCount = 2,
                BackColor = Color.FromArgb(155, 162, 178),
                Margin = new Padding(0, 0, 0, 4),
            ...
            ctrlOuter.Controls.Add(new Label
            {
                ...
                ForeColor = Color.FromArgb(14, 20, 40),
                BackColor = Color.FromArgb(135, 142, 160),
                Dock = DockStyle.Fill,
                TextAlign = ContentAlignment.MiddleCenter
            }, 0, 0);
            ...
            OpenDoorButton = MakeCtrlBtn("◀▶   OPEN DOOR", Color.FromArgb(22, 130, 50));
            CloseDoorButton = MakeCtrlBtn("▶◀   CLOSE DOOR", Color.FromArgb(68, 82, 115));
            UpButton = MakeCtrlBtn("↑   UP", Color.FromArgb(68, 82, 115));
            DownButton = MakeCtrlBtn("↓   DOWN", Color.FromArgb(68, 82, 115));
            ...
            StartButton = MakeSysBtn("▶  START", Color.FromArgb(22, 92, 185));
            PauseButton = MakeSysBtn("⏸  PAUSE", Color.FromArgb(175, 122, 12));
            StopButton = MakeSysBtn("⏹  STOP", Color.FromArgb(175, 34, 34));
        ...
        internal void LayoutFloorButtons()
        {
            ...
            // 2-column grid: floors 4,3 on top row; 2,1 on bottom row
            int cols = 2, rows = 2;
            int btnW = Math.Max(28, (pw - pad * 2 - gap * (cols - 1)) / cols);
            int btnH = Math.Max(28, (ph - pad * 2 - gap * (rows - 1)) / rows);

            // Make buttons square (use the smaller dimension)
            ...
            // Layout: row 0 = floors 4,3 (left to right); row 1 = floors 2,1
            ...
            for (int i = 0; i < floorOrder.Length; i++)
            {
                ...
                // Make circular via Region (re-apply each layout in case size changed)
